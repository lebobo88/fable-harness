...
**N8 — No MCP, no headless.** FABLE-HARNESS runs only through interactive Claude Code CLI sessions (`claude`, `claude --worktree`, `claude --resume`, etc.). It never registers an MCP server and never depends on `-p`/print/headless invocation as part of its own operation. Local CLI tools and hook scripts are fine, since they're only ever invoked from within an interactive session.
